...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    if [ -n "$SYS_API_ROUTE" ]; then
        echo "sys.apiRoute: ${SYS_API_ROUTE:-"\"direct\""}" >> "$CONFIG_FILE"
    fi
    if [ -n "$SYS_PROXY" ]; then
        echo "sys.proxy: ${SYS_PROXY:-"\"\""}" >> "$CONFIG_FILE"
    fi
    if [ -n "$SYS_LANGUAGE" ]; then
        echo "sys.language: ${SYS_LANGUAGE:-"\"cn\""}" >> "$CONFIG_FILE"
    fi
    if [ -n "$SYS_THEME" ]; then
        echo "sys.theme: ${SYS_THEME:-"\"multiverse\""}" >> "$CONFIG_FILE"
    fi
    ...
    if [ -n "$BIU_DOWNLOAD_MODE" ]; then
        echo "biu.download.mode: ${BIU_DOWNLOAD_MODE:-"\"dl-single\""}" >> "$CONFIG_FILE"
    fi
    ...
    if [ -n "$BIU_DOWNLOAD_WHATS_UGOIRA" ]; then
        echo "biu.download.whatsUgoira: ${BIU_DOWNLOAD_WHATS_UGOIRA:-"\"webp\""}" >> "$CONFIG_FILE"
    fi
